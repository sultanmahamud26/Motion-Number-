...
def start_bot(pid):
    ...
    try:
        ...
        # mainbot নিজে restart/crash হলেও child-এর আসল OS PID যেন হারিয়ে না
        # যায় — তাই disk-এ (data.json) সেভ করে রাখা হচ্ছে। এটাই মূল ফিক্স:
        # mainbot restart হলেও এই pid দিয়ে child bot খুঁজে বের করে বন্ধ করা যাবে।
        d0 = load()
        if pid in d0["projects"]:
            d0["projects"][pid]["os_pid"] = proc.pid
            save(d0)
...
def _pid_belongs_to_project(os_pid, proj_dir):
    """
    /proc/<pid>/cwd বা cmdline চেক করে নিশ্চিত হয় যে এই OS PID সত্যিই এই
    project-এর process, নাকি PID reuse হয়ে অন্য কোনো ভিন্ন প্রসেস এখন এই
    নাম্বারে বসে আছে (নাহলে ভুল প্রসেস kill হয়ে যাওয়ার ঝুঁকি থাকে)।
    """
    try:
        cwd_link = f"/proc/{os_pid}/cwd"
        if os.path.exists(cwd_link):
            real_cwd = os.path.realpath(cwd_link)
            return os.path.realpath(proj_dir) == real_cwd
    except Exception:
        pass
    return False
...
def _kill_process(pid):
    ...
    # ১) mainbot নিজের মেমরিতে যদি এখনো process handle থাকে (normal case)
    with _proc_lock:
        entry = processes.get(pid)
    ...
    # ২) mainbot restart/crash হয়ে মেমরি হারিয়ে গেলে — data.json-এ সেভ করা
    # আসল OS PID দিয়ে fallback kill করার চেষ্টা করো (এটাই মূল ফিক্স)
    data = load()
    saved_pid = data["projects"].get(pid, {}).get("os_pid")
    if saved_pid:
        try:
            if _pid_belongs_to_project(saved_pid, proj_dir):
                os.kill(saved_pid, signal.SIGTERM)
                time.sleep(1)
                # এখনো বেঁচে থাকলে জোর করে kill
                if _pid_belongs_to_project(saved_pid, proj_dir):
                    os.kill(saved_pid, signal.SIGKILL)
                killed = True
        except ProcessLookupError:
            pass  # প্রসেসটা আগে থেকেই বন্ধ ছিল, সমস্যা নেই
        except Exception:
            pass
...
def stop_bot(pid):
    ...
    if pid in data["projects"]:
        ...
        data["projects"][pid]["os_pid"] = None
        save(data)
    return True

def is_running(pid):
    # ১) mainbot-এর নিজের মেমরিতে live handle থাকলে সেটাই সবচেয়ে নির্ভরযোগ্য
    with _proc_lock:
        entry = processes.get(pid)
        if entry and entry.get("process"):
            return entry["process"].poll() is None

    # ২) mainbot restart হয়ে মেমরি হারিয়ে গেলে — data.json-এর সেভ করা OS PID
    # দিয়ে চেক করো process আসলেই এখনো বেঁচে আছে কিনা (এটাই মূল ফিক্স,
    # নাহলে restart-এর পর সব child-কে "বন্ধ" মনে হতো যদিও আসলে চলছিল)
    data = load()
    proj = data["projects"].get(pid, {})
    saved_pid = proj.get("os_pid")
    if saved_pid and proj.get("status") == "running":
        proj_dir = os.path.join(PROJECTS_DIR, pid)
        try:
            os.kill(saved_pid, 0)  # শুধু existence চেক, kill করে না
            return _pid_belongs_to_project(saved_pid, proj_dir)
